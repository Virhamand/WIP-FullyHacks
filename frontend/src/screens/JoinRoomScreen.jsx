...
export default function JoinRoomScreen({ onJoin, onBack }) {
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (!canSubmit) return; onJoin(name.trim(), code.toUpperCase()); };

  const inputStyle = { width: '100%', background: 'rgba(3,18,42,0.75)', border: '1px solid rgba(56,189,248,0.25)', borderRadius: '0.5rem', padding: '0.875rem 1rem', color: '#e0f2fe', fontFamily: MONO, fontSize: '0.9rem', backdropFilter: 'blur(8px)', boxSizing: 'border-box', transition: 'border-color 0.2s' };
        ...
        <div style={{ width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', animation: 'fadeUp 0.6s 0.1s ease-out forwards', opacity: 0 }}>
            ...
            <input
              ...
              placeholder="ROOM"
              maxLength={4}
              style={{ ...inputStyle, textAlign: 'center', fontSize: '2rem', letterSpacing: '0.3em', fontWeight: 700, color: code.length === 4 ? '#38bdf8' : '#e0f2fe', textShadow: code.length === 4 ? '0 0 20px rgba(56,189,248,0.5)' : 'none' }}
            />
            <button
              type="submit"
              disabled={!canSubmit}
              style={{ width: '100%', padding: '0.875rem', background: canSubmit ? 'linear-gradient(135deg,#0369a1,#0284c7)' : 'rgba(3,18,42,0.5)', border: `1px solid ${canSubmit ? '#38bdf8' : 'rgba(56,189,248,0.12)'}`, borderRadius: '0.5rem', color: canSubmit ? '#e0f2fe' : 'rgba(125,211,252,0.3)', fontFamily: MONO, fontWeight: 700, fontSize: '0.9rem', letterSpacing: '0.05em', cursor: canSubmit ? 'pointer' : 'not-allowed', transition: 'all 0.2s', boxShadow: canSubmit ? '0 0 20px rgba(56,189,248,0.2)' : 'none' }}
            >
              Dive in
            </button>
          </form>
