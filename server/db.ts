...
export async function updateRoundStatus(roundId: number, status: "answering" | "pointing" | "done") {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable");
  await db.update(gameRounds).set({ status }).where(eq(gameRounds.id, roundId));
}
